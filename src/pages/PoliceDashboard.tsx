...
const PoliceDashboard: React.FC = () => {
    ...
    try {
      ...
      if (response.data.success) {
        const apiAlerts = response.data.data.map((alert: any) => ({
          ...
          status: alert.status,
          touristName: alert.touristId ? `${alert.touristId.personalInfo?.firstName || ''} ${alert.touristId.personalInfo?.lastName || ''}`.trim() : undefined,
          touristId: alert.touristId?.touristId
        }));
        setActiveAlerts(apiAlerts);
      }
    } catch (error) {
    ...
    try {
      ...
      if (response.data.success) {
        const apiTourists = response.data.data.map((tourist: any) => ({
          ...
          location: tourist.location?.current?.address || 'Unknown location',
  ...
  const handleAlertAction = async (alertId: string, action: 'investigate' | 'resolve') => {
    try {
      if (action === 'investigate') {
        await apiClient.put(`/api/alerts/${alertId}/acknowledge`, {
          notes: 'Under investigation by police'
        });
      } else {
        await apiClient.put(`/api/alerts/${alertId}/resolve`, {
          resolution: 'Resolved by police officer',
          notes: 'Alert handled and resolved'
        });
      }
      // Update local state
